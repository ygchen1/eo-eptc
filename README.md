...
If you want to follow the full pipeline:

### Step 1: Feature Extraction

```bash
python 01feature_extract.py
```

* Input: raw `pcapng` files
* Output: extracted flow-level feature files

You need to download the full `pcapng` dataset in advance and adjust paths in the script as needed.

### Step 2: Flow Pairing

```bash
python 02flow_merge.py
```

* Pairs `original_flow` and `proxied_flow` based on the extracted features.

### Step 3: Build Dataset Cache

```bash
python 03dataset_cache.py
```

* Aggregates features to generate intermediate cache files in `data_cache/`.

### Step 4: Split Dataset

```bash
python 04dataset_split.py
```

* Splits the dataset into:
  * **seq2seq** dataset
  * **seqclassify** dataset
* Uses a specified category ratio (e.g., 30/30).
...
1. Train the **seq2seq model** on the seq2seq dataset
2. Use the trained model to generate translated sequences for the seqclassify dataset
3. Train/evaluate the **classification model** and output performance metrics
